...
export default function Home() {
  ...
  useEffect(() => {
    isMountedRef.current = true;  // ← FIX: reset on mount so StrictMode doesn't break us
          ...
          if (payload.eventType === 'INSERT' && payload.new?.channel === 'team') {
            setTeamChatMessages(prev => {
              ...
              return [...prev, {
                ...
                avatar: payload.new.sender_avatar || '👤', text: payload.new.body,
  ...
  // ============================================================================
  // UNIFIED VOICE RECORDING — granular error handling, StrictMode-safe
  // ============================================================================
  const cleanupStream = () => {
  ...
  const processAiSubmission = async (userSubmissionInput) => {
    ...
    setBotChatMessages(prev => [...prev, {
      id: Date.now().toString(), sender: botName || 'Aura-1', avatar: '🤖',
  ...
  const handleClearChat = () => {
    if (chatTab === 'bot') {
      if (window.confirm('Are you sure you want to delete all workspace messages?')) {
        setBotChatMessages([{ id: 'bot-clear-1', sender: botName || 'Aura-1', avatar: '🤖', text: 'Workspace chat cleared. How can I assist you with this step?', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), type: 'system' }]);
      }
    } else if (myTeamId) {
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-4 sm:p-6 font-sans space-y-6 min-h-screen relative">
      ...
      <div className="bg-slate-900/40 border border-cyan-500/20 rounded-3xl p-5 flex flex-wrap items-center justify-between gap-5 shadow-2xl shadow-cyan-500/10 backdrop-blur-2xl relative">
        ...
        {(earnedMedal || earnedTitle) && (
          <div className={`flex items-center space-x-2.5 px-4 py-2 rounded-xl border font-mono text-sm font-bold ${getMedalColor(earnedMedal || 'Diamond')} backdrop-blur-sm`}>
            ...
            {earnedTitle && (<><span className="text-slate-500">|</span><span className="text-white font-bold">🏆 {earnedTitle}</span></>)}
          </div>
        )}
          ...
          {isTeamMode && (
            <>
              ...
              <div className="relative">
                ...
                {showTeamMembers && (
                  <div className="absolute right-0 mt-2 w-80 bg-slate-900/90 border border-cyan-500/30 rounded-2xl p-4 shadow-2xl shadow-cyan-500/20 z-50 space-y-3 font-mono text-xs backdrop-blur-2xl">
                          ...
                          <div key={m.id} className="flex flex-col space-y-2 bg-slate-800/60 p-3 rounded-xl border border-slate-700">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-2">
                                <div className="w-8 h-8 rounded-full bg-slate-900 flex items-center justify-center text-lg border border-slate-700">{m.avatar || '👤'}</div>
                ...
                return (
                  <div key={msg.id || index} className={`flex flex-col space-y-1 ${isUser ? 'items-end' : 'items-start'}`}>
                    <div className="flex items-center space-x-2 text-[10px] text-slate-400 px-1 font-extrabold">
                      <span>{msg.avatar} {msg.sender}</span><span>•</span><span>{msg.time}</span>
                      {isUser && chatTab === 'bot' && (
